...
func takesNoEscapeClosure(_ fn : () -> Int) {
  ...
  doesEscape { x() }    // expected-error {{closure use of non-escaping parameter 'x' may allow it to escape}}
...
enum r19997577Type {
  ...
  func reduce<Result>(_ initial: Result, _ combine: @noescape (Result, r19997577Type) -> Result) -> Result { // expected-error 2{{unknown attribute 'noescape'}}
    // expected-note@-1 {{parameter 'combine' is implicitly non-escaping}}
    ...
    // expected-error@-1 {{closure use of non-escaping parameter 'combine' may allow it to escape}}
    switch self {
    case .Unit:

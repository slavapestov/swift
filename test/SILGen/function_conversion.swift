...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion7TrivialVSgACIxyd_AcDIxyd_TR : $@convention(thin) (Trivial, @owned @callee_owned (Optional<Trivial>) -> Trivial) -> Optional<Trivial>
// CHECK:         enum $Optional<Trivial>
// CHECK-NEXT:    apply %1(%2)
// CHECK-NEXT:    enum $Optional<Trivial>
// CHECK-NEXT:    return

// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion7TrivialVSgACIxyd_A2DIxyd_TR : $@convention(thin) (Optional<Trivial>, @owned @callee_owned (Optional<Trivial>) -> Trivial) -> Optional<Trivial>
// CHECK:         apply %1(%0)
// CHECK-NEXT:    enum $Optional<Trivial>
// CHECK-NEXT:    return
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion8LoadableVSgACIxxo_A2DIxxo_TR : $@convention(thin) (@owned Optional<Loadable>, @owned @callee_owned (@owned Optional<Loadable>) -> @owned Loadable) -> @owned Optional<Loadable>
// CHECK:         apply %1(%0)
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion8AddrOnlyVSgACIxir_A2DIxir_TR : $@convention(thin) (@in Optional<AddrOnly>, @owned @callee_owned (@in Optional<AddrOnly>) -> @out AddrOnly) -> @out Optional<AddrOnly>
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion1Q_pAA7TrivialVIxid_AdA1P_pIxyr_TR : $@convention(thin) (Trivial, @owned @callee_owned (@in Q) -> Trivial) -> @out P
// CHECK:         alloc_stack $Q
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion1Q_pSgAA7TrivialVIxid_AESgAA1P_pIxyr_TR
// CHECK:         switch_enum
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion1Q_pAA7TrivialVIxid_AA1P_pAaE_pIxir_TR : $@convention(thin) (@in P, @owned @callee_owned (@in Q) -> Trivial) -> @out P
// CHECK:         alloc_stack $Q
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion1Q_pXmTSgAA7TrivialVXMtIxyd_AEXMtAA1P_pXmTIxyd_TR : $@convention(thin) (@thin Trivial.Type, @owned @callee_owned (Optional<@thick Q.Type>) -> @thin Trivial.Type) -> @thick P.Type
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion1Q_pXmTSgAA7TrivialVXMtIxyd_AEXMtSgAA1P_pXmTIxyd_TR : $@convention(thin) (Optional<@thin Trivial.Type>, @owned @callee_owned (Optional<@thick Q.Type>) -> @thin Trivial.Type) -> @thick P.Type
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion1Q_pXmTSgAA7TrivialVXMtIxyd_AA1P_pXmTAaF_pXmTIxyd_TR : $@convention(thin) (@thick P.Type, @owned @callee_owned (Optional<@thick Q.Type>) -> @thin Trivial.Type) -> @thick P.Type
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion6ParentCXMTAA7TrivialVSgAA5ChildCXMTIxyyd_AHXMTAeCXMTIxyyd_TR : $@convention(thin) (@thick Child.Type, Trivial, @owned @callee_owned (@thick Parent.Type, Optional<Trivial>) -> @thick Child.Type) -> @thick Parent.Type
// CHECK:         upcast %0 : $@thick Child.Type to $@thick Parent.Type
// CHECK:         apply
// CHECK:         upcast {{.*}} : $@thick Child.Type to $@thick Parent.Type
// CHECK:         return

// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion6ParentCXMTSgAA7TrivialVSgAA5ChildCXMTIxyyd_AIXMTAfCXMTIxyyd_TR : $@convention(thin) (@thick Child.Type, Trivial, @owned @callee_owned (Optional<@thick Parent.Type>, Optional<Trivial>) -> @thick Child.Type) -> @thick Parent.Type
// CHECK:         upcast %0 : $@thick Child.Type to $@thick Parent.Type
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T0ypS2iIxyd_Ixio_S2iIxyd_ypIxxr_TR : $@convention(thin) (@owned @callee_owned (Int) -> Int, @owned @callee_owned (@in Any) -> @owned @callee_owned (Int) -> Int) -> @out Any
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T0S2iIxyd_S2iIxir_TR : $@convention(thin) (@in Int, @owned @callee_owned (Int) -> Int) -> @out Int
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion6ParentCxAA7TrivialVIxxod_xAcESgIxxod_ACRbzlTR : $@convention(thin) <T where T : Parent> (@owned T, @owned @callee_owned (@owned Parent) -> (@owned T, Trivial)) -> (@owned Parent, Optional<Trivial>)
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion6ParentCXMTxXMTAA7TrivialVIxydd_xXMTACXMTAESgIxydd_ACRbzlTR : $@convention(thin) <T where T : Parent> (@thick T.Type, @owned @callee_owned (@thick Parent.Type) -> (@thick T.Type, Trivial)) -> (@thick Parent.Type, Optional<Trivial>)
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T019function_conversion1Q_pIxi_AA1P_pIxi_TR : $@convention(thin) (@in P, @owned @callee_owned (@in Q) -> ()) -> ()

// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T0Si_SitSgIxy_S2iIxyy_TR : $@convention(thin) (Int, Int, @owned @callee_owned (Optional<(Int, Int)>) -> ()) -> ()
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T0S3iIxydd_S2i_SitSgIxyd_TR : $@convention(thin) (Int, @owned @callee_owned (Int) -> (Int, Int)) -> Optional<(Int, Int)>
...
// CHECK:       sil shared [transparent] [fragile] [reabstraction_thunk] @_T0xxxIxirr_xx_xtSgIxir_lTR : $@convention(thin) <T> (@in T, @owned @callee_owned (@in T) -> (@out T, @out T)) -> @out Optional<(T, T)>
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T0s11AnyHashableVABSbIxiid_xxSbIxiid_s0B0RzlTR : $@convention(thin) <T where T : Hashable> (@in T, @in T, @owned @callee_owned (@in AnyHashable, @in AnyHashable) -> Bool) -> Bool
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T0Ix_ypIxr_TR : $@convention(thin) (@owned @callee_owned () -> ()) -> @out Any
// CHECK:         init_existential_addr %0 : $*Any, $()
// CHECK-NEXT:    apply %1()
// CHECK-NEXT:    tuple ()
// CHECK-NEXT:    return

// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T0Ix_ypSgIxr_TR : $@convention(thin) (@owned @callee_owned () -> ()) -> @out Optional<Any>
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T0S2iIxdd_ypIxr_TR : $@convention(thin) (@owned @callee_owned () -> (Int, Int)) -> @out Any
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T0S2iIxdd_ypSgIxr_TR : $@convention(thin) (@owned @callee_owned () -> (Int, Int)) -> @out Optional<Any> {
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T0ypIxi_S2iIxyy_TR : $@convention(thin) (Int, Int, @owned @callee_owned (@in Any) -> ()) -> ()
...
// CHECK-LABEL: sil shared [transparent] [fragile] [reabstraction_thunk] @_T0ypSgIxi_S2iIxyy_TR : $@convention(thin) (Int, Int, @owned @callee_owned (@in Optional<Any>) -> ()) -> ()
// CHECK:         [[ANY_VALUE:%.*]] = alloc_stack $Any
// CHECK-NEXT:    [[ANY_PAYLOAD:%.*]] = init_existential_addr [[ANY_VALUE]]
// CHECK-NEXT:    [[LEFT_ADDR:%.*]] = tuple_element_addr [[ANY_PAYLOAD]]
// CHECK-NEXT:    store %0 to [trivial] [[LEFT_ADDR]]

...
ConstructorDecl *SwiftDeclConverter::importConstructor(
  ...
  // Constructors use a special DeclBaseName, and not the Identifier
  // `init`.
  assert(!importedName.getDeclName().isSimpleName());
  auto declName = DeclName(Impl.SwiftContext,
                           DeclBaseName::createConstructor(),
                           importedName.getDeclName().getArgumentNames());

  // Create the actual constructor.
  auto result = Impl.createDeclWithClangNode<ConstructorDecl>(
      objcMethod, AccessLevel::Public, declName,

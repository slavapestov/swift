...
void SILGenModule::maybeMarkAsFragile(SILFunction *f, IsFragile_t fragile) {
  // If we're not referencing the thunk from a fragile context, there is
  // nothing to do.
  if (!fragile)
    return;

  SmallVector<SILFunction *, 2> worklist;

  auto addToWorklist = [&](SILFunction *f) {
    // Only valid for thunks with shared linkage.
    assert(f->isThunk() && "Non-thunks must be emitted with the correct "
           "fragility ahead of time");
    assert(f->getLinkage() == SILLinkage::Shared &&
           "only necessary for thunks with shared linkage");

    // If the thunk is already fragile, there is nothing to do.
    if (f->isFragile())
      return;

    // Mark it fragile -- we won't visit it again.
    f->setFragile(IsFragile);

    // Visit any thunks referenced within.
    worklist.push_back(f);
  };

  // Add the initial function to the worklist.
  addToWorklist(f);

  // Process entries until all referenced functions have been marked as
  // fragile.
  while (!worklist.empty()) {
    // Get the next function to process.
    auto *otherF = worklist.back();
    worklist.pop_back();

    // Walk all basic blocks to find FunctionRefInsts contained therein.
    for (auto &BB : *otherF) {
      for (auto &I : BB) {
        if (auto *FRI = dyn_cast<FunctionRefInst>(&I)) {
          auto *refF = FRI->getReferencedFunction();

          // If refF is public or shared [fragile], we don't have to
          // visit it's body recursively.
          if (!refF->hasValidLinkageForFragileRef())
            addToWorklist(refF);
        }
      }
    }
...
getOrCreateReabstractionThunk(GenericEnvironment *genericEnv,
  ...
  auto *f = M.getOrCreateSharedFunction(loc, name, thunkType, IsBare,
                                        IsTransparent, Fragile,
                                        IsReabstractionThunk);

  // Mark it as fragile if needed.
  maybeMarkAsFragile(f, Fragile);

  if (Fragile)
    f->dump();
  assert(!Fragile || f->isFragile());

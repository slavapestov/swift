...
void SILGenFunction::emitNativeToForeignThunk(SILDeclRef thunk) {
  ...
  if (thunk.hasDecl()) {
    ...
    if (auto attr = decl->getAttrs().getAttribute<ObjCAttr>()) {
      // If @objc was inferred based on the Swift 3 @objc inference rules,
      // emit a call to Builtin.swift3ImplicitObjCEntrypoint() to enable
      // runtime logging of the uses of such entrypoints.

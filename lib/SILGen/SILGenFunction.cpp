...
void SILGenFunction::emitCurryThunk(ValueDecl *vd,
  ...
  if (isa<ConstructorDecl>(vd) || isa<EnumElementDecl>(vd)) {
    ...
    // FIXME: This is very similar to emitConstructorMetatypeArg()
    auto selfMetaTy = vd->getInterfaceType()
                        ->castTo<AnyFunctionType>()->getInput();
    selfMetaTy = ArchetypeBuilder::mapTypeIntoContext(vd->getInnermostDeclContext(),
                                                      selfMetaTy);

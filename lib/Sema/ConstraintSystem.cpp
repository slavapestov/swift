...
void ConstraintSystem::assignFixedType(TypeVariableType *typeVar, Type type,
  ...
  // Callers should have already rejected constraints that would violate the
  // type variable's inherent constraints.
  assert(typeVar->getImpl().canBindToInOut() || !type->is<InOutType>());
  assert(typeVar->getImpl().canBindToLValue() || !type->hasLValueType());
  assert(typeVar->getImpl().canBindToNoEscape() || !type->isNoEscape());

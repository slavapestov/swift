...
template <typename Runtime>
struct TargetDefaultWitnessTableEntry {
  TargetRelativeDirectPointer<Runtime, void *> Ptr;
};
...
struct TargetProtocolDescriptor {
  ...
  using DefaultWitnessTableEntry = TargetDefaultWitnessTableEntry<Runtime>;

  DefaultWitnessTableEntry DefaultWitnessTable[];

  /// Default requirements are tail-allocated here.
  void *getDefaultWitness(unsigned index) const {
    return DefaultWitnessTable[index].Ptr.get();
  }
